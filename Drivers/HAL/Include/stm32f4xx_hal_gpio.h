...
/**
 * @brief           Определение структуры данных для настройки GPIO
 */
struct gpio_config {
    uint32_t mode;                              /*!< Режим работы @ref enum gpio_mode */

    uint32_t output_type;                       /*!< Тип вывода @ref enum gpio_output_type */

    uint32_t output_speed;                      /*!< Скорость работы вывода @ref enum gpio_output_speed */

    uint32_t pupd;                              /*!< Подтяжка сигнала @ref enum gpio_pupd */
...
struct gpio_handle {
    ...
    uint32_t pin;                               /*!< Номер порта ввода-вывода GPIO @ref gpio_pin */
};
